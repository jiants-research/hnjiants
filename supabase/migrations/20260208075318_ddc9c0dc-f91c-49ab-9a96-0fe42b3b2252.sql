...
-- Add user_id column to open_loops
ALTER TABLE public.open_loops
  ADD COLUMN user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;

-- Drop existing permissive policies on open_loops
DROP POLICY IF EXISTS "Allow all read access to open_loops" ON public.open_loops;
DROP POLICY IF EXISTS "Allow all update access to open_loops" ON public.open_loops;

-- Create authenticated RLS policies for open_loops
